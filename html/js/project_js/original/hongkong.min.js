  ...
  try {
    ...
    if (
      ...
      (d = staticObject.fromSun(d)),
        $('#playerDividend').text(
          parseFloat(d).toFixed(2).toLocaleString('en') + ' Tron'
        );
      ...
      (c =
        0 === l
          ? '<tr><td>' +
            ...
            '</td><td>' +
            d +
            ' Tron</td><td style="color:#ff5959">-</td></tr>'
          : '<tr><td>' +
            t +
            '</td><td>' +
            horseNames[n] +
...
async function startLiveevents() {
  try {
    var e = await staticObject.Tron.getContract(gameLocation1Contract),
...
async function liveMyBets() {
  if (
    ...
    try {
      var e = await window.tronWeb.trx.getContract(gameLocation1Contract),
        t = await window.TronWeb.contract(e.abi.entrys, e.contract_address),
        n = await window.TronWeb.defaultAddress.hex;
      t.RaceResult().watch({ filter: { _bettor: n } }, (e, t) => {
        var n = window.TronWeb.address.fromHex(t.result._bettor);
        n = n.substring(0, 5) + '...' + n.substr(n.length - 5);
        var a,
          o = t.result._horseNum,
          r = t.result._p1,
