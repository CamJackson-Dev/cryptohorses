...
async function initGlobalData() {
  ...
  (global.userSigned = e.sign), getPlayerLevel();
}
...
function getTronscanName(e) {
  return new Promise(function (t, n) {
    ...
    $.ajax({
      ...
      async: !0,
      success: function (n) {
        (r = '' != n.name ? n.name : getUserAddress(e)), t(r);
      },
      error: function (n, o, a) {
        (r = getUserAddress(e)), t(r);
      },
    });
  });
}
async function updateWonAmont() {
  try {
    (response = await $.get(`${url}/wonAmount`)),
      (odometer.innerHTML = parseInt(response.totalPaidAmt / 1e6));
  } catch (e) {}
}
$(document).ready(async function () {
  ...
  setTimeout(function () {
    setInterval(updateWonAmont, 30e3);
  }, 6e3);
...
$('.changeChatRoom').on('click', function () {
  ...
  socket.on('getMessage', async ({ address: e }) => {
    try {
      (await $.get(`${url}/chat/${roomName}`)).chats.forEach(
        ({ userName: t, userAddress: n, message: r, level: o }) => {
          var a = jQuery(
              `#${
                e === n
  ...
  socket.on('newMessage', function ({ userName: e, message: t, level: n }) {
    ...
    var o = jQuery(`#${r}`).html(),
      a = getUserameClass(n),
      s = Mustache.render(o, { color: a, userName: e, message: t, image: '1' });
  ...
  jQuery('#all-chats-form').on('submit', async (e) => {
    if (
      ...
      if (0 == global.userSigned) {
        let r = !1;
        try {
          r = (await $.get(`${url}/check/player/${global.userAddress}`)).flag;
        } catch (e) {}
        if (!r)
          return (
            Toastify({
        ...
        try {
          ...
          var r,
            ...
            a = $('.textarea-chat-inputfield').text();
          r = (r =
            o.length <= 2 && '' != a
              ? o + a
              : o.length >= 4 && !o.includes(a)
              ? o + a
              : o).substring(0, 127);
          const e = {
