...
const tW = require('Tronweb'),
  ...
  staticObject = new tW(TronNode, TronNode, TronNode, privateKey);
...
function autoTronLinkloginCheck() {
  let e = 0;
  window.addEventListener('TronWebInjected', { once: !0 }), a;
  const t = setInterval(() => {
    const { TronWeb: n } = window;
    if (++e > 4)
      return (
        window.removeEventListener('TronWebInjected', { once: !0 }),
        clearInterval(t)
      );
    n &&
      n.ready &&
...
async function TronLinkloginCheck() {
  let e = 0;
  window.addEventListener('TronWebInjected', { once: !0 });
  const t = setInterval(() => {
    const { TronWeb: n } = window;
    if (++e > 4)
      return (
...
async function initGlobalData() {
  ...
  (global.userSigned = e.sign), getPlayerLevel(), initContractInstance();
  try {
    const e = await $.get(`${url}/getProfile/${global.userAddress}`);
    if (
      ('success' == e.msg &&
        ($('#levelPlayer').text(e.data.level),
        $('#playerRank').text(Math.ceil(e.data.level / 5)),
        $('#totalBets').text(e.data.totalBets.toLocaleString('en')),
        $('#totalWinna').text(e.data.totalWinna.toLocaleString('en')),
        $('#totalEarning').text(
          e.data.comulativeEarning.toLocaleString('en') + ' Tron'
        ),
        $('#lastDrop').text(e.data.lastWinDrop + ' Tron')),
      document.getElementById('my-profile-bets-body'))
    ) {
      (filter = 'all'), (skip = 0), (selected = 1), (count = 0);
      let e = await $.get(
        `${url}/hongkong/filter/${global.userAddress}?filter=${filter}&page=${page}&skip=${skip}`
      );
      (myBets = e.location1bets), (count = e.count);
      const t = Math.ceil(parseFloat(count / page));
      $('#skip').children('a').remove();
      for (let e = 1; e <= t; e++)
        $('#skip').append(
          $('<a></a>')
            .attr({ class: 'dropdown-item skipChange', href: '#', name: e })
            .text(e)
        );
      0 !== myBets.length &&
        ...
        myBets.forEach(
          ({
            ...
            var i = jQuery('#my-profile-bets').html(),
              s = Mustache.render(i, {
                order: t,
                hash: n,
                hashString: n.slice(0, 13),
                horsePrediction: horseNames[a[0].horse],
                winningHorse:
                  '[ 1st: ' +
                  horseNames[e[0]] +
                  ', 2nd: ' +
                  ...
                  horseNames[e[2]] +
                  ' ]',
                betAmount: (a[0].win + a[0].place + a[0].show) / 1e6,
                winAmount: 0 === o ? '-' : o / 1e6,
                color: 0 === o ? 'color:#ff5959' : 'color:#01F593',
                createdAt: new Date(r).toLocaleString(),
              });
            jQuery('#my-profile-bets-body').append(s);
          }
        );
    }
  } catch (e) {
...
function getTronscanName(e) {
  return new Promise(function (t, n) {
    var a = '';
    $.ajax({
      url: 'https://apilist.Tronscan.org/api/account?address=' + e,
      dataType: 'json',
      async: !0,
      success: function (n) {
...
let intervalID = setInterval(async function () {
  if ('object' == typeof window.TronWeb)
    try {
      var e = await window.TronWeb.defaultAddress.base58,
        t = await window.TronWeb.defaultAddress.hex;
      global.userAddress,
        '' != global.userAddress &&
...
async function updateMintInfo() {
  try {
    var e = await tokenContractInstanceStatic.stage().call(),
      ...
      i = staticObject.fromSun(n.totalMintLimit),
      s = staticObject.fromSun(n.mintedTillNow),
      l = staticObject.fromSun(a[1]);
    if (
      (a[0] || 0 == l
        ? $('#availableWinnaDrop').text(
            parseFloat(l).toFixed(2).toLocaleString('en') + ' Tron'
          )
        : $('#availableWinnaDrop').text(
            '-' + parseFloat(l).toFixed(2).toLocaleString('en') + ' Tron'
          ),
      ...
      var c = await dividendContractInstanceStatic
        .availableDividendIndividualLive(window.TronWeb.defaultAddress.base58)
        .call();
      (c = staticObject.fromSun(c)),
        $('#playerDividend').text(
          parseFloat(c).toFixed(2).toLocaleString('en') + ' Tron'
        );
    } else $('#playerDividend').text('0 Tron');
    $('#stage').text(e),
      $('#level').text(t),
      $('#difficulty').text(r + ' Tron'),
      $('#mintedtillNow').text(parseFloat(s).toFixed(2).toLocaleString('en')),
      $('#mintLimit').text(parseFloat(i).toLocaleString('en'));
    var d = (s / i) * 100 + '%';
    $('#progressBar').css({ width: d });
  } catch (e) {
    console.log('error', e);
  }
}
$('.walletDropDown').on('click', function () {
  ...
  'guild-wallet' == e
    ...
    : (setCookie('wallet', e, 5),
      $('#currWallet').html('<img src="images/' + e + '.png">TronLink'));
}),
    ...
    try {
      ...
      if (
        ...
        var a,
          o,
          r,
          i = new Date(1e3 * globalInfo.withdrawTime),
          s = new Date(i).getTime(),
          l = new Date(i).toLocaleString();
        $('#unfreezeDate').text(l);
        var c = s - new Date().getTime();
        (a = Math.floor((c % 864e5) / 36e5)),
          (o = Math.floor((c % 36e5) / 6e4)),
...
async function initContractInstance() {
  if (
    ...
    try {
      var e = await window.tronWeb.trx.getContract(tokenContract);
      tokenContractInstance = await window.TronWeb.contract(
        e.abi.entrys,
        e.contract_address
      );
      var t = await window.tronWeb.trx.getContract(dividendContract);
...
async function initInstanceStatic() {
  try {
    var e = await staticObject.Tron.getContract(tokenContract);
    tokenContractInstanceStatic = await staticObject.contract(
      e.abi.entrys,
      e.contract_address
    );
    var t = await staticObject.Tron.getContract(dividendContract);
...
function waitForTxConfirmation(e) {
  return new Promise(function (t, n) {
    var a = setInterval(async function () {
      try {
        var n = await tronWeb.Tron.getTransactionInfo(e);
        n &&
...
function waitForTxConfirmationEvent(e, t) {
  return new Promise(function (n, a) {
    var o = setInterval(async function () {
      try {
        var a = await tronWeb.getEventByTransactionID(e);
        if (a.length >= 1)
...
setTimeout(function () {
  ...
  $('#freezeConfirm').on('click', async function () {
    if (
      ...
      var e = $('#frzAmt').val();
      try {
        var t = await window.TronWeb.defaultAddress.base58;
        if (
          (
...
$('.changeChatRoom').on('click', function () {
  ...
  socket.on('newMessage', function ({ userName: e, message: t, level: n }) {
    ...
    var o = jQuery(`#${a}`).html(),
      r = getUserameClass(n),
      i = Mustache.render(o, { color: r, userName: e, message: t, image: '1' });
    jQuery('#messages').append(i), scrollToBottom();
  }),
  jQuery('#all-chats-form').on('submit', async (e) => {
    if (
      ...
      if (0 == global.userSigned) {
        ...
        var t = window.TronWeb.toHex('CryptoHorseRacing.com');
        t = t.substring(2);
        try {
          var n = await window.TronWeb.Tron.sign(t);
          '' != n &&
            socket.emit(
              'signChat',
  ...
  $('.locationChange').on('click', function () {
    var e = $(this).attr('name');
    locationChange(e), $('#currLocation').text($(this).text());
  }),
  $('.filterChange').on('click', function () {
    var e = $(this).attr('name');
    filterChange(e), $('#currFilter').text($(this).text());
  }),
  $('#skip').on('click', '.skipChange', function () {
    var e = $(this).attr('name');
    skipChange(e), $('#currSkip').text(e);
  }),
  $('.pageChange').on('click', function () {
    var e = $(this).attr('name');
    pageChange(e), $('#currPage').text($(this).text());
  });
const locationChange = (e) => {},
  filterChange = async (e) => {
    ...
    (myBets = t.location1bets), (count = t.count);
    const n = Math.ceil(parseFloat(count / page));
    $('#skip').children('a').remove();
    for (let e = 1; e <= n; e++)
      $('#skip').append(
        $('<a></a>')
