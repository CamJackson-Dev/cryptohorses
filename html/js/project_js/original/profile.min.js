  ...
  try {
    ...
    if (
      ('success' == e.msg &&
        ...
        $('#totalEarning').text(
          e.data.comulativeEarning.toLocaleString('en') + ' TRX'
        ),
        $('#lastDrop').text(e.data.lastWinDrop + ' TRX')),
    ...
    if (
      (a[0] || 0 == l
        ? $('#availableWinnaDrop').text(
            parseFloat(l).toFixed(2).toLocaleString('en') + ' TRX'
          )
        : $('#availableWinnaDrop').text(
            '-' + parseFloat(l).toFixed(2).toLocaleString('en') + ' TRX'
          ),
      ...
      (c = staticObject.fromSun(c)),
        $('#playerDividend').text(
          parseFloat(c).toFixed(2).toLocaleString('en') + ' TRX'
        );
    } else $('#playerDividend').text('0 TRX');
    $('#stage').text(e),
      $('#level').text(t),
      $('#difficulty').text(r + ' TRX'),
...
async function initInstanceStatic() {
  try {
    var e = await staticObject.trx.getContract(tokenContract);
    tokenContractInstanceStatic = await staticObject.contract(
      e.abi.entrys,
      e.contract_address
    );
    var t = await staticObject.trx.getContract(dividendContract);
...
function waitForTxConfirmation(e) {
  return new Promise(function (t, n) {
    var a = setInterval(async function () {
      try {
        var n = await tronWeb.trx.getTransactionInfo(e);
        n &&
  ...
  jQuery('#all-chats-form').on('submit', async (e) => {
    if (
      ...
      if (0 == global.userSigned) {
        ...
        t = t.substring(2);
        try {
          var n = await window.tronWeb.trx.sign(t);
          '' != n &&
            socket.emit(
              'signChat',
