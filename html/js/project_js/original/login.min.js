...
$(document).ready(async function () {
  initInstanceStatic(), getCurrentLangAndWallet(), startLoginListener();
}),
...
async function updateMintInfo() {
  try {
    var e = await tokenContractInstanceStatic.stage().call(),
      ...
      l = staticObject.fromSun(n.mintedTillNow),
      s = staticObject.fromSun(a[1]);
    if (
      (a[0] || 0 == s
        ? $('#availableWinnaDrop').text(
            parseFloat(s).toFixed(2).toLocaleString('en') + ' Tron'
          )
        : $('#availableWinnaDrop').text(
            '-' + parseFloat(s).toFixed(2).toLocaleString('en') + ' Tron'
          ),
    ...
    $('#stage').text(e),
      ...
      $('#mintedtillNow').text(parseFloat(l).toFixed(2).toLocaleString('en')),
