function CBetPanel(){
    ...
    this.onStartExit = async function(){
       if(_iTotBet < MIN_BET){
            ...
            try{
                ...
                instance = await tronWeb.contract(contractInfo.abi.entrys, contractAddress);
                ...
                let connection = await instance.GoodLuckWINNA(betData, playerSeed, uniqueString).send(args);
                ...
                async function checkStatus(){
                    try{
                        ...
                        if(event.length >= 1){
                            var findIndex = event.findIndex(eventArray => eventArray.name === "RaceResult");
                            if(findIndex != -1){
                                clear();
                                var aRank = new Array();
                                ...
                                aRank.push(parseInt("0x" + leaderBoardencoded.slice(961, 1024)));
                                console.log(aRank)

                                s_oBetPanel.updateTRXBalance();
                                s_oBetPanel.unload();
                        ...
                        // var status = await tronWeb.trx.getTransactionInfo(connection);
                        // console.log(status)
                        // if(status){
    ...
    // setInterval(this.updateTRXBalance, 3000)

    this.updateTRXBalance = async function(){
        try{
            var trxBalance = await tronWeb.trx.getBalance(tronWeb.defaultAddress.base58);
            s_iCurMoney = ( trxBalance / 1000000).toFixed(2);
        }catch(e){
