...
exports.distributeDividend = async () => {
    try{
        ...
        var dividendContractInfo = await tronWeb.trx.getContract(config.DIVIDEND_CONTRACT_ADDRESS);
        dividendContractInstance = await tronWeb.contract(dividendContractInfo.abi.entrys, dividendContractInfo.contract_address);
        
        var systemHalt = await dividendContractInstance.changeDivSystemStatus().send({
        ...
        if(responseHalt){
            ...
            var distributeTx = await dividendContractInstance.distributeDividend().send({
                shouldPollResponse: false,
                feeLimit: 5000000
            })
            ...
            if(isDividendAvailable){
                ...
                try{
                    var totalNumberOfStackers = await dividendContractInstance.getMaxStackerIndex().call();
                    ...
                    console.log("There are " + totalNumberOfStackers + " stackers. Total " + totalPossibleTransaction + " transaction needs to be done.");
                    for(var txNo=1; txNo<=totalPossibleTransaction; txNo++){
                        console.log('for loop: ' + txNo)
                        var max;
                        if(txNo == totalPossibleTransaction){ max = totalNumberOfStackers - 1;}
                        else { max = (txNo * 10) - 1; }
                        ...
                        try{
                            ...
                            var accounts = await dividendContractInstance.getLimitedStackers(min, max).call();
                            var payDivTX = await dividendContractInstance.payDivUser(accounts).send({
                                shouldPollResponse: false
                            });
                    ...
                    try{
                        ...
                        var divPaidNo = await dividendContractInstance.numberOfDividendPaidTillNow().call()
                        ...
                        mainhouse = await MainHouse.findByIdAndUpdate(
                            mainhouse.id,
                            {
                                totalDividendPaid: mainhouse.totalDividendPaid + totalDistributed,
