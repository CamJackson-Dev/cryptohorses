...
/*
 ...
 contract WinnaToken is Ownable, Erc20Token{
    ...
    mapping(address => uint) public claim;
    mapping(address => uint) public totalBet;

    uint public miningInterval = 20000000;
    uint public miningDays = 0;
    ...
    uint256[] private miningDifficultyArray = [
      ...
      400000000, 420000000, 440000000, 460000000, 480000000,
      500000000, 520000000, 540000000, 560000000, 580000000];

    uint256[] public refBounds = [//trx spent for each level - each value should be higher than the last (cumulative)
      1000000, 5000000, 1000000, 1000000, 1000000, 1000000, 1000000, 1000000, 1000000, 1000000,
    ...
    uint[10] refRewards = [100, 101, 102, 103, 104, 105, 106, 107, 108, 110]; //percentage point increase in mining rewards per 10 levels.

    uint public freezeCoolDown = 0 minutes;

    uint public releaseInterval = 1 days;
    uint public releaseDate = now + 1 days;

    uint adminBalance = 0;

    uint referralCount = 5000100;
    mapping (uint => address) public referralOwners;
    mapping (address => uint) public referralClaim;
    mapping (address => uint) public referralUsers;
    mapping (address => uint) public referralWithdrawn;
    mapping (address => uint) public referralLinks;
    mapping (address => address) public referee;

  mapping(uint => dividendStage) public stages;
  ...
  function setCoolDown(uint time) onlyOwner public{
      freezeCoolDown = time;
  }

  function setReleaseInterval(uint time) onlyOwner public{
      releaseInterval = time;
  }

  function addToWhitelist(address ad) onlyOwnerOrServer public{
      whitelist[ad] = !whitelist[ad];
  }

  function deposit() public payable{
     stages[currentStage].stageBalance = stages[currentStage].stageBalance.add(msg.value);
  }
  ...
  function deposit(address player, uint betAmount) public payable{
    ...
    uint _amount = betAmount.mul(1000000).div(miningDifficultyArray[currentStage]);
    ...
    stages[currentStage].stageBalance = stages[currentStage].stageBalance.add(msg.value.mul(4).div(10));
  ...
  function addStage() public returns (bool){
    require(releaseDate < now);
      currentStage++;
      stages[currentStage] = dividendStage(stages[currentStage - 1].totalFrozen, 0);
  ...
  function freeze(uint amount) public{
    ...
    stages[currentStage].totalFrozen = stages[currentStage].totalFrozen.add(amount);
  }

  function unfreeze(uint amount) public{
    require(frozen[msg.sender].time  + freezeCoolDown < now && frozen[msg.sender].amount >= amount);
    updateClaim(msg.sender);
    frozen[msg.sender].amount = frozen[msg.sender].amount.sub(amount);
    balances[msg.sender] = balances[msg.sender].add(amount);
    stages[currentStage].totalFrozen = stages[currentStage].totalFrozen.sub(amount);
  }
  ...
  function updateClaim(address ad) public{
      ...
      if(frozen[msg.sender].stage < currentStage){
          for(uint x = frozen[ad].stage; x < currentStage; x++){
            if(stages[x].totalFrozen != 0){
              uint stagePayout = frozen[ad].amount.mul(stages[x].stageBalance).div(stages[x].totalFrozen);
    ...
    function updateClaimSafe(address ad) public{
      ...
      if(frozen[msg.sender].stage + 10 < currentStage){
          for(uint x = frozen[ad].stage; x < frozen[ad].stage + 10; x++){
            if(stages[x].totalFrozen != 0){
              uint stagePayout = frozen[ad].amount.mul(stages[x].stageBalance).div(stages[x].totalFrozen);
              payout = payout.add(stagePayout);
            }
  ...
  function getClaim(address user) public view returns(uint){
    uint payout = 0;
    if(frozen[user].stage == 0 || frozen[user].amount == 0){
      payout = 0;
    }
    else if(frozen[msg.sender].stage < currentStage){
        for(uint x = frozen[user].stage; x < currentStage; x++){
          if(stages[x].totalFrozen != 0){
            uint stagePayout = frozen[user].amount.mul(stages[x].stageBalance).div(stages[x].totalFrozen);
            payout = payout.add(stagePayout);
          }
        }
    }
  ...
  function getLevel(address player) public returns(uint level){
  ...
  function addToClaim(address user, uint amount) public onlyOwnerOrServer{
